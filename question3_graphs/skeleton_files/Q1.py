...
def list_of_vertices(filename):
    ...
    with open(filename, mode ='r')as file:
        ...
        for lines in csvFile:
            if (lines[0],float(lines[len(lines)-1])) not in vertices:
                vertices.append((lines[0],float(lines[len(lines)-1])))
...
def list_of_edges(filename):
    ...
    with open(filename, mode ='r')as file:
        ...
        for lines in csvFile:
            # print(eval(lines))
            if ((lines[0],lines[1],eval(lines[2]),eval(lines[3]),lines[4])) not in Edges:
                Edges.append((lines[0],lines[1],eval(lines[2]),eval(lines[3]),lines[4]))
